...
class MarketplaceService2 {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedBody is List) {
          final posts = decodedBody.map((json) => MarketplacePost.fromJson(json)).toList();
          ...
          if (decodedBody['data'] is List) {
            ...
            _postsCache[cacheKey] = posts;
            return posts;
          } else if (decodedBody['data'] == 'Please accept live auction terms') {
  ...
  Future<bool> acceptAuctionTerms(String userId) async {
    final url = '$baseUrl/live-auction-terms-accept.php?token=$token&user_id=$userId';
    try {
      final response = await http.get(Uri.parse(url));
      if (response.statusCode == 200) {
        final decodedBody = jsonDecode(response.body);
...
Future<Map<String, String>> fetchPostAttributeValues(String postId) async {
  final url = '$baseUrl/post-attribute-values.php?token=$token&post_id=$postId';
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedBody['status'] == 'true' && decodedBody['data'] is List) {
        ...
        for (var item in decodedBody['data']) {
          if (item['attribute_name']?.isNotEmpty == true && item['attribute_value']?.isNotEmpty == true) {
            uniqueAttributes[item['attribute_name']] = item['attribute_value'];
          }
        }
